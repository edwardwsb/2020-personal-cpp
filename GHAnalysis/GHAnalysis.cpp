...
void init(string path) {
	...
	for (auto u : res) {
		const char* p = u.data();
		FILE* stream1;
		freopen_s(&stream1, p, "r", stdin); cout << p << endl;
		while (getline(cin, s)) {
			cout << ++kk << endl;
			cout << (int)s.size() << endl;
			bool f1 = 0; bool f2 = 0; bool f3 = 0; string name = "";	string person = ""; string type = "";
			for (int i = 0; i <= (int)s.size() - 1; i++) if (s[i] == '\"') {
				i++;  string now = "";
				while (s[i] != '\"') {
					now += s[i++];
				}
	...
	FILE* stream2;
	freopen_s(&stream2, "1.json", "w", stdout);
	for (auto u : personnum) {
		cout << u.first << " " << u.second << endl;
	}
	freopen_s(&stream2, "2.json", "w", stdout);
	for (auto u : namenum) {
		cout << u.first << " " << u.second << endl;
	}
	freopen_s(&stream2, "3.json", "w", stdout);
	for (auto u : totnum) {
		cout << u.first << " " << u.second << endl;
	}
}
...
void getvoid1(string& person, string& type) {
	FILE* stream1;
	freopen_s(&stream1, "1.json", "r", stdin);
	string s1;
	long long s2;
	while (cin >> s1) {
...
void getvoid2(string& name, string& type) {
	FILE* stream1;
	freopen_s(&stream1, "2.json", "r", stdin);
	string s1;
	long long s2;
	while (cin >> s1) {
...
void getvoid3(string& person, string& name, string& type) {
	FILE* stream1;
	freopen_s(&stream1, "3.json", "r", stdin);
	string s1;
	long long s2;
	while (cin >> s1) {
		cin >> s2; personnum[s1] = s2;
